...
function JuniorHistoryInfoPage() {
  ...
  const handleJuniorHistoryConfirm = async () => {
    if (!ticketId) {
      console.error("예매 정보가 없습니다.");
      return;
    }

    try {
      const response = await fetch(
        `${API_BASE_URL}/api/movie/sendTicket?ticketId=${ticketId}`,
        {
          method: "GET",
        },
      );
      ...
      if (!response.ok) {
        ...
        console.error("❌ 문자 전송 실패:", errorText);
        console.error("문자 전송에 실패했습니다.");
        return;
      }

      console.log("✅ 문자 전송 성공");
      navigate("/juniorHistoryConfirm");
    } catch (error) {
      console.error("❌ 예외 발생:", error);
      console.error("문자 전송 중 오류가 발생했습니다.");
    }
  };
    ...
    <div className="bg-customBg h-screen text-white flex flex-col relative mx-auto w-[560px] min-w-[560px] max-w-[560px]">
      ...
      <footer className="fixed bottom-0 w-[560px] bg-gray-800 flex mx-auto">
        ...
        <button
          ...
          onClick={handleJuniorHistoryConfirm}
        >
          예매 내역 전송하기
        </button>
      </footer>
...
export default JuniorHistoryInfoPage;
