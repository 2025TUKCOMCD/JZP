...
# 디버깅: 문자 집합 크기 확인
print(f"num_classes: {num_classes}, char_set length: {len(char_set)}")
...
label_length = np.array([np.count_nonzero(label) for label in labels])  # 라벨 실제 길이 계산
...
print(f"input_length shape: {input_length.shape}")
print(f"label_length shape: {label_length.shape}")

# 디버깅: 라벨과 라벨 길이 확인
for i, label in enumerate(labels[:5]):  # 첫 5개 샘플만 출력
    print(f"Sample {i}: label length = {label_length[i][0]}, label = {label}")

# 디버깅: 모델 출력 예상 크기
y_pred_shape = (len(images), images.shape[2] // 4, num_classes)  # 예측 크기
print(f"Expected y_pred shape: {y_pred_shape}")
